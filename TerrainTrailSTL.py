...
def GenerateSTLs(Boundary=[],Rect_Pt=[],rd_include=[],trail_exclude=[],waterway_include=[],waterbody=[],trail_gpx=[],
    ...
    if not map_only:
        if len(compass_loc)==2:
           # compass_bounds=np.array([[8,38],[25,0],[0,-25],[-25,0],[-8,38]])*compass_size #polygon that bounds default compass size.
           # compass_bounds=shp.geometry.Polygon(compass_bounds+np.array(compass_loc))
           # x,y=np.meshgrid(dem.lon,dem.lat)
           # x=x.flatten()
           # y=y.flatten()
           # cords=np.vstack((x,y)).transpose()
           # cords=cord2dist(cords,corner=corner,f=scale_factor)
           # pts=shp.geometry.MultiPoint(cords)
        ...
        if len(wb_msh)>0:
            ...
            wb_msh[1]=tm.boolean.union([wb_msh[2],terrain_all])
